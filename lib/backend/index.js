...
// TODO: Add investment alternatives such as gold, fixed deposit account, etc.
const ALTERNATIVES = mongoose.model("ALTERNATIVES", mongoose.Schema({
    _id: String,
    sm_close: Number,
    oy_close: Number,
    ty_close: Number,
...
app.get('/api/stonks/alternatives/:id', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    ALTERNATIVES.find({ "_id": req.params.id }).exec(function (err, data) {
        if (err)
            console.log(err);
        res.json(data);
    });
...
app.put('/api/stonks/alternatives/:id', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    ALTERNATIVES.updateOne({"_id": req.params.id}, {$set: {
        "sm_close": req.body.sm_close,
        "oy_close": req.body.oy_close,
        "ty_close": req.body.ty_close,
        "fy_close": req.body.fy_close,
...
app.get('/api/stonks/users', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
...
// Change to put
app.post('/api/stonks/users/makeAdmin', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    USER.updateOne({"_id": req.body.username}, {$set: {
        "is_admin": true
    }}).exec(function (err, data) {
        if (err) console.log(err);
        else res.send("success");
...
// Change to put
app.post('/api/stonks/users/removeAdminStatus', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    USER.updateOne({"_id": req.body.username}, {$set: {
        "is_admin": false
    }}).exec(function (err, data) {
...
app.delete('/api/stonks/users/delete/:id', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    USER.deleteOne({"_id": req.params.id}).exec(function (err, data) {
        if (err) console.log(err);
        else res.send("success");
    });
});
...
app.get('/api/stonks/articles_for/:id', function (req, res) {
    res.header("Access-Control-Allow-Origin", "*");
    ARTICLES.find({ "for_symbol": req.params.id }).sort('-timestamp').exec(function (err, data) {
        if (err)
            console.log(err);
        res.json(data);
    });
...
app.post('/api/stonks/add_symbol', function (req, res) {
    ...
    const symbol = new SYMBOLS({
        ...
        one_year_dividend: req.body.one_year_dividend,
        two_year_dividend: req.body.two_year_dividend,
        three_year_dividend: req.body.three_year_dividend,
        four_year_dividend: req.body.four_year_dividend,
        five_year_dividend: req.body.five_year_dividend
    });
    console.log(symbol);
...
app.put('/api/stonks/change_password',  function (req, res) {
    ...
    USER.findOne({ "_id": req.body.username }).exec(function (err, data) {
        ...
        bcrypt.compare(req.body.old_password, data.password, function(err, result) {
            if (err) res.send("error");
            if(result) {
                bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
                    if(err) res.send("error");
                    USER.updateOne({"_id": req.body.username}, {$set: {"password": hash}}).exec(function (err, data){
                        if(err)
                            console.log(err);
...
MARKETSTANDARDS.find({}).exec(function (err, data) {
    ...
    for (i = 0; i < collections.length; i++) {
        const ANALYSIS = mongoose.model(collections[i], mongoose.Schema({
            ...
            sm_duration_high: Number,
            sm_duration_low: Number,
            oy_close: Number,
            oy_date: String,
            ...
            oy_duration_high: Number,
            oy_duration_low: Number,
            ty_close: Number,
            ty_date: String,
            ...
            ty_beta: Number,
            ty_duration_high: Number,
            ty_duration_low: Number,
            fy_close: Number,
            fy_date: String,
            ...
            fy_duration_high: Number,
            fy_duration_low: Number,
        }));
